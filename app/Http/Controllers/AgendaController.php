...
class AgendaController extends Controller
{
    ...
    public function list (Request $request) {
        ...
        if(isset($data['zoom_auth']) ) {
            ...
            $querySelect= Agendas::select('id','title','description','description_large','duration_time','start_at','timezone','audience_config','resources','category_id','price','zoom_code');
        }
    ...
    public function live (Request $request) {
        ...
        foreach($query as $meeting) {
            if($meeting->category->name != 'Taller_M' && $meeting->category->name != 'Taller_T'  && $meeting->category->name != 'PRUEBAS') {
                $start = date('Y-m-d H:i', strtotime('-5 min',$meeting->start_at));
