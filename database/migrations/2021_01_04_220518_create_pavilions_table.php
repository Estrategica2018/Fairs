...
class CreatePavilionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pavilions', function (Blueprint $table) {
            ...
            $table->integer('stands_number')->comment('Código único del festival, evento o campaña.')->unsigned();
            $table->integer('rooms_number')->comment('Número de stands en el pabellón')->unsigned();
            $table->longText('resources')->comment('Número de salas del pabellón')->unsigned();
            $table->timestamps();
        });
    }
